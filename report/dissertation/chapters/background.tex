...
The fourth is context. A camera trap is fixed, so the same scene recurs in every clip from that site. That
gives a model a shortcut: it can learn the background rather than the animal, which works until the site
changes~\cite{terra,panaf,xiao}.
