...
difficulties that make animal detection and tracking hard in the first place, and how tracking is scored. It
then turns to the literature on estimating a model's performance without labels, what is known about
distribution shift in camera-trap vision, the representations that make a distance between species
meaningful, and the datasets involved. It closes by stating the gap that remains.
...
Wild footage is unusually difficult input, and the difficulties are set out here because they are the
conditions any deployed tracker must handle.

The first is that animals are frequently difficult to see. Many are camouflaged against the habitat they live
...
The second is variability of appearance. A camera trap is triggered by any passing animal, so an animal may
occupy most of the frame in one clip and a few dozen pixels in the next, may be viewed frontally or only in
profile, and is frequently partially occluded by vegetation. A bounding box is a poor description of an animal in
...
is sufficiently difficult in wildlife imagery to constitute a research area in its own right~\cite{animalreid}.
...
threshold $\alpha$ it forms a detection term (\texttt{DetA}: whether the correct objects were found) and an
association term (\texttt{AssA}: whether identities were kept consistent over time), then combines them as a
geometric mean:
\begin{equation}
...
calibrates a confidence threshold on source data and reads off target accuracy as the mass above it.
\citet{autoeval} instead regress classifier accuracy directly on a distribution-shift feature, the Fr\'echet
...
Background therefore constitutes signal rather than noise. \citet{elephant} make the parallel point for detectors. Transplanting an object into an unfamiliar scene
suppresses its detection, flips its predicted identity as it moves, and disturbs other detections in the
image, exposing a reliance on context and co-occurrence.

\citet{xiao} also report that more accurate and more robust models rely on background \emph{less}, which
suggests the effect is not fixed but a property of the model's overall accuracy and robustness.

The literature shares a common orientation. It measures degradation after the fact, or intervenes
...
Vision foundation models encode that structure. BioCLIP~\cite{bioclip} shows that a model trained
...
Two datasets are central to this work: SA-FARI, on which the primary study runs, and BURST, which carries the
independent replication.

\subsection{SA-FARI}
\label{sec:bg-safari}
SA-FARI~\cite{safari} is a large open multi-animal camera-trap tracking dataset,
...
\subsection{BURST}
\label{sec:bg-burst}
BURST~\cite{burst} is a video benchmark for object recognition, segmentation and tracking,
...
documented, but not forecast, location-driven degradation~\cite{terra,wilds,panaf}. A recent unified study
of camera-trap recognition over time~\cite{jeon} explicitly identifies ``predicting when zero-shot models
will succeed at a new site'' as an open question, but does not address it.
