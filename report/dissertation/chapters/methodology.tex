...
\begin{equation}
  ...
  \label{eq:protodist}
\end{equation}
where $e$ indexes the query's own species. That exclusion is the leave-species-out rule again. A
CLIP~\cite{clip} encoder replaces DINOv2 as a robustness check. Because the masks come from the annotations,
this feature never runs SAM\,3 (Figure~\ref{fig:features}b).
...
For a cell recorded in year $y$, it is the smallest absolute difference to any reference year in $\mathcal{Y}$,
that is $d_{\mathrm{temp}}=\min_{r\in\mathcal{Y}}|y-r|$. Years come from the per-video creation-datetime
metadata.

This axis turns out not to be testable here. A usable timestamp is present for only $32$ of the $1{,}447$ cells
($2.2\%$), and all $32$ have a gap of exactly zero. With no variance to exploit, the temporal axis is reported
as untested rather than as a tested null.
\subsection{Animal size (confound control)}
This covariate measures how large the animal appears in frame.

It is the log mean ground-truth mask area of a species,
$\mathrm{log\_area}(s)=\ln\!\big(1+\overline{\mathrm{area}(s)}\big)$, averaged over sampled masklets. Larger
animals are easier to segment. Size is therefore what separates genuine visual novelty from mere
conspicuousness, which makes it a nuisance covariate rather than a hypothesis variable. Its role in exposing
the one apparent effect as an artefact is Section~\ref{sec:meth-model}.
