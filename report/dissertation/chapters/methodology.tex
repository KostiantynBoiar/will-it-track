...
\texttt{pDetA} and \texttt{pAssA} are tested here.

\paragraph{Interpreting the replication.} BURST is held to the same bar as SA-FARI and carries the same
animal-size positive control (Section~\ref{sec:meth-cv}). Whether that control fires is what decides how a flat
result on BURST may be read. Either way the replication can show convergence rather than independent proof,
because a dataset of this size cannot exclude a small effect (Section~\ref{sec:burst}).
