...
that criterion~(i) tests, and the third the out-of-sample comparison that criterion~(ii) tests. The three that
follow test no hypothesis. They exist so that a wrong conclusion cannot be drawn from whatever the first three
return: they divide explained variance between predictors that overlap, ask whether a second family of
label-free properties fares better, and check that the outcome does not hinge on any single design choice.
