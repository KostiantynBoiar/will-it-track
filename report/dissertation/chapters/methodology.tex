...
dataset and two further trackers, and the constraints that make the work reproducible.
...
each measuring how far a target species or place sits from a frozen reference. None of them requires the
...
fixed object of study. Three terms require definition first: what running it zero-shot means, what the two
...
model is handed a small number of labelled examples. A zero-shot model receives only the term
\texttt{``impala''}.
...
fitted anywhere in this study is the small regression mapping distances onto scores. Zero-shot is meant
relative to the dataset and to the reference frozen inside it, not as a claim about SAM\,3's undisclosed
...
identity can only be assessed for an animal that was found in the first place. A second consequence follows.
Where a clip holds a single animal there is only one identity to keep, so
...
(Section~\ref{sec:meth-inference}). A distance is taken to predict transfer only if it satisfies both criteria:
\begin{enumerate}[label=(\roman*),leftmargin=2.4em]
  \item its coefficient's group-cluster-bootstrap 95\% confidence interval (CI) excludes zero; and
  \item the grouped cross-validated model improves on a mean-predictor baseline by a margin that is significant under
...
Criterion~(ii) is that test, and its baseline is deliberately minimal: predicting each held-out cell with the
...
under tests built deliberately to be powerful, the conclusion is H0: distance explains little variance. That is
a substantive result rather than a failure to find one, and it motivates the representational-probing direction
of Section~\ref{sec:concl-future}.
...
to define our reference distribution, which permits the same videos to be repartitioned. Inference is run
...
The replication and the model swap (Sections~\ref{sec:meth-replication} and~\ref{sec:meth-swap}) later re-run
the same pipeline on other datasets and trackers.
...
(Section~\ref{sec:meth-zeroshot}), reusing that single inference pass across both splits is sound rather than
a leak. A cell's score cannot depend on which reference partition is later overlaid to compute its distances.
...
of Section~\ref{sec:meth-familiarity} is included as a check against that gap.
...
then computed against that reference alone. This is the leakage safeguard: no information about a probe can
enter a feature. A unit test verifies it, swapping the reference and checking that the distances move as
expected. The axis that is strictly disjoint is asserted --- location for Split~B, the held-out species for
Split~A --- and any residual species overlap is reported rather than assumed absent.
...
One qualification applies. As implemented on an annotated benchmark, the visual,
...
would be built from the small number of exemplar images a practitioner already holds for a named species. It
does bound the claim, and Section~\ref{sec:limitations} returns to it.
...
\end{minipage}
\caption[The image-based distances]{The two image-based distances on a camera-trap frame (a collared
peccary). \textbf{(a)} ground-truth mask (orange); \textbf{(b)} the mask-cropped animal that the visual
distance embeds; \textbf{(c)} the animal mean-inpainted, which the environment distance embeds. None of these
features runs SAM\,3.}
\label{fig:features}
\end{figure}
...
$\ln(1+a)$. Larger animals are easier to segment, so it distinguishes visual novelty from apparent
...
(Section~\ref{sec:meth-measure}). The proxy addresses that gap by deriving familiarity from the model itself.
The same
...
scene covariates, and a $\log(n_\text{frames})$ term. The last of these prevents low support from being
mistaken for high distance.
...
unit-tested to recover planted effects on synthetic data. If the procedure validates the size covariate while
returning no gain for the distances, a null result is evidence of absence of signal rather than absence of
power. Chapter~\ref{ch:results} reports whether it does, and a planted-effect analysis quantifies the
smallest effect this test could have detected (Appendix~\ref{app:supplementary}, Table~\ref{tab:mde}).
...
apparent difficulty effect that is in fact attributable to size (Table~\ref{tab:difficulty},
Appendix~\ref{app:supplementary}). Each scene property's cell-level
...
The pipeline is bound to an annotation schema rather than to a loader, so an offline adapter converting the
new dataset into the SA-Co JSON the loader and evaluator already read is all that changes.
...
leave-species-out cross-validation. SAM\,3 is replaced by another open-vocabulary, text-promptable tracker. Only \texttt{pDetA} changes, because the distances are functions of the data and its ground truth
...
average-precision metric with no gate at all. This gate also decides which dataset can host a fair swap, and
is why the swap runs on BURST rather than SA-FARI (Chapter~\ref{ch:results} gives the measurements). Each
challenger configuration is fixed in advance from the model's own defaults, run once, and reported as-is
...
Each is enforced as a configuration toggle, so an ablation changes a config file and nothing else. The
pipeline is covered by hermetic tests, including the leakage check of Section~\ref{sec:meth-features} and
the recovery of planted effects on synthetic data. Split definitions,
random seeds and reference sets are persisted; every reported number comes from the pipeline, and both
experiments reproduce end to end on a single GPU.
