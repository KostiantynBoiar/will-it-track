...
not predict SAM~3's per-cell detection/association to a degree any valid test supports. The powered species
hold-out (99 species, 1025 cells) confirmed this null and, tellingly, traced the one wrong-signed
``significant'' effect to an animal-size confound rather than novelty. This is a legitimate scientific
result that sharpens the next question.
\end{abstract}
...
\subsection{Gate 2 --- species hold-out (Split A, powered): a null, and a size confound}
The primary novelty test --- leave-one-species-out over the \emph{pooled} train+test species (99 species,
1025 scored cells, after a capped SAM~3 pass over the train split) --- is in Table~\ref{tab:coefA}. The
out-of-sample gain is again \textbf{not significant} on any hold-out (indeed slightly negative: species
\pdeta{} $\Delta$MAE $-0.004$, $p=0.74$). With the extra power one coefficient does reach significance ---
but it is \emph{visual distance}, and \textbf{positive} (\pdeta{} $+0.369$ $[+0.11, +0.67]$): visually
\emph{novel} species are tracked \emph{better}, the opposite of H1. That is the fingerprint of a
\textbf{confound}. Raw correlations show animal size ($\log$-area) is the strongest single correlate of the
score (\pdeta{} $r=0.30$), and that visually-distinctive species are larger
(visual~$\leftrightarrow$~$\log$-area $r=0.22$); taxonomic novelty is flat ($r\approx0.00$). Add a size
covariate and the visual coefficient collapses to $+0.221$ $[-0.006, +0.431]$ (\pdeta{}, CI now spanning
zero) / $+0.148$ $[-0.067, +0.356]$ (\passa{}), while size itself becomes the significant driver
($\log$-area $+0.274$ $[+0.04, +0.51]$ / $+0.240$ $[+0.03, +0.45]$). So ``visual novelty helps'' was really
``bigger animals are easier to segment''. \textbf{Novelty does not predict transfer, even powered.}
...
$\log(\text{support})$ & $-0.020$ $[-0.14,\,0.16]$ & $+0.051$ $[-0.06,\,0.23]$ \\
\bottomrule
\end{tabular}
\caption{Split A (species hold-out, pooled train+test: 99 species, 1025 cells) standardised coefficients
with group-cluster-bootstrap CIs. Only \emph{visual distance} excludes zero, and with the wrong (positive)
sign --- an animal-size confound: controlling for size collapses it to $+0.22$ with a CI spanning zero.}
\label{tab:coefA}
\end{table}
...
On both the location split \emph{and} the powered species hold-out (99 species, 1025 cells), the label-free
distances do \textbf{not} predict SAM~3's per-cell detection or association to a degree any valid,
group-clustered test supports. This is \textbf{H0}. The one effect that reaches significance under power ---
visual distance --- has the \emph{wrong} sign and dissolves into an animal-size confound (previous
subsection); taxonomic novelty is flat throughout. The only robust qualitative signal is that
...
\begin{enumerate}[leftmargin=1.6em,itemsep=6pt]
  ...
  \item \textbf{The strong-looking effects are artifacts that vanish under scrutiny.}
        On the location split the taxonomic distance looked important ($-0.36$), but that split makes
        novelty degenerate --- almost all test species also occur in the reference set, so only $\sim$7
        genuinely-unseen species sit above zero and the ``$-0.36$'' was a fragile contrast driven by those
        few. On the graded species axis it becomes $+0.20$ with a CI spanning zero (Table~\ref{tab:coefA})
        --- no graded novelty effect. The one coefficient that \emph{does} reach significance under power,
        visual distance, has the wrong (positive) sign and dissolves into an animal-size confound once size
        is controlled ($+0.37 \to +0.22$, CI spanning zero). Every apparent signal evaporates when the
        design or a confounder is fixed --- exactly what you expect when there is no real effect.
...
\addlinespace
Scoring the train predictions was OOM-killed: the evaluator loads the \emph{entire} train ground-truth
(all $\sim$31k videos, 0.9\,GB) to score our $\sim$1500 probes. &
Trim the ground-truth to just the predicted videos before the evaluator sees it --- it never needs the
rest --- keeping its memory at test-split levels. \\
\addlinespace
