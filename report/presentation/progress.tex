...
%  The modelling half: an honest NEGATIVE result.
...
% ------------------------------------------------------------ what the pipeline sees (figure)
\begin{frame}{What the pipeline sees --- a real frame}
  \centering
  \begin{columns}[c]
    \begin{column}{0.31\textwidth}\centering
      \includegraphics[width=\linewidth]{../dissertation/figures/feat_overlay.png}\\[2pt]{\scriptsize frame $+$ ground-truth mask}
    \end{column}
    \begin{column}{0.31\textwidth}\centering
      \includegraphics[width=\linewidth]{../dissertation/figures/feat_animal.png}\\[2pt]{\scriptsize \textbf{visual}: the animal alone}
    \end{column}
    \begin{column}{0.31\textwidth}\centering
      \includegraphics[width=\linewidth]{../dissertation/figures/feat_background.png}\\[2pt]{\scriptsize \textbf{environment}: animal erased}
    \end{column}
  \end{columns}
  \vspace{0.5em}
  {\footnotesize The distances are built from label-free crops of a real camera-trap frame (a collared
  peccary): the animal's appearance (\emph{visual}), the scene with the animal erased (\emph{environment}),
  plus its place on the tree of life and its timestamp. \textbf{No SAM\,3 is used to build them.}}
\end{frame}

% ------------------------------------------------------------ the null (table + figure)
  ...
  \begin{columns}[c]
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{lcc}
        ...
        \midrule
        guess the average  & $0.30$ & --- \\
        $+$ four distances & $0.30$ & \no\ \textbf{no} \\
        $+$ animal size    & $0.28$ & \ck\ a little \\
        \bottomrule
      \end{tabular}\\[8pt]
      {\scriptsize out-of-sample error, held-out species / places (lower is better)}
    \end{column}
    \begin{column}{0.54\textwidth}
      \centering
      \includegraphics[width=0.9\linewidth]{../dissertation/figures/res_pred_vs_actual.pdf}\\[-1pt]
      {\scriptsize predictions collapse onto the \emph{mean}, not the diagonal}
    \end{column}
  \end{columns}
\end{frame}
...
% ------------------------------------------------------------ ablations / variance
\begin{frame}{The null survives every check}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \footnotesize \textbf{Drop any one distance} --- the fit and the (null) out-of-sample gain barely move.
      Only \textbf{adding animal size} changes anything.
      \vspace{2pt}
      ...
      \begin{tabular}{lcc}
        ...
        \midrule
        all four distances & $0.055$        & \no\ no \\
        drop any one       & $0.04$--$0.06$ & \no\ no \\
        $+$ animal size    & $0.119$        & \ck\ yes \\
        \bottomrule
      \end{tabular}
      \end{table}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{itemize}\footnotesize
        \item The four distances together explain only $\approx\textbf{3\%}$ of the variance.
        \item No factor dominates, and every collinearity score is $\approx1$ --- so it is \textbf{not}
              collinearity hiding a real effect.
        \item The null is not an artefact of one factor or one modelling choice.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Where we are, and what's next}
  \begin{block}{Done \& validated}
    ...
    difficulty pivot \ck\;\; false positives \ck\;\; ablations \& variance \ck\;\; write-up \ck
  \end{block}
  \vspace{0.5em}
  \begin{itemize}\footnotesize
    \item \textbf{Write-up:} results, discussion, and figures now reflect the negative result and its
          explanation; the dissertation builds end-to-end.
    \item \textcolor{burnt}{\textbf{In progress:}} three robustness re-runs (a different visual encoder,
          whole-frame crops, a generic prompt) --- awaiting a GPU; the null is expected to hold.
    \item \textbf{Honest limitation:} distances are to the \emph{dataset's} training split, not SAM\,3's true
          (undisclosed) training data. \textbf{Open direction:} probe whether SAM\,3's own features
          ``know'' a species.
  \end{itemize}
  \vspace{0.3em}
